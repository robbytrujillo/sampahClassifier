...
function showResult(predictions, fromCamera = false) {
  ...
  predictions.forEach((p) => {
    ...
    `;
    detailText += `${p.className}: ${percent}%\n`;
  });
  ...
  lastResult = {
    ...
    details: detailText,
    time: new Date().toLocaleString(),
    image: snapshot?.toDataURL("image/png") || null,
  };
...
//   pdf.setFontSize(14);
//   pdf.text("LAPORAN HASIL DETEKSI SAMPAH", 105, y, { align: "center" });

//   y += 15;
//   pdf.setFontSize(11);
//   pdf.text(`Tanggal : ${lastResult.time}`, 15, y);

//   y += 10;
//   pdf.text("Hasil Deteksi:", 15, y);

//   y += 8;
//   pdf.text(`Label               : ${lastResult.label}`, 20, y);
//   y += 7;
//   pdf.text(`Confidence          : ${lastResult.confidence}%`, 20, y);
//   y += 7;
//   pdf.text(`Hashing Coefficient : ${lastResult.hc}%`, 20, y);

//   y += 12;
//   pdf.text("Detail Probabilitas:", 15, y);

//   y += 8;
//   pdf.setFontSize(10);

//   lastResult.details.split("\n").forEach((line) => {
//     if (line.trim()) {
//       pdf.text(line, 20, y);
//       y += 6;
//     }
...
async function exportPDF() {
  if (!lastResult || !lastResult.image) {
    alert("⚠️ Belum ada hasil untuk diexport");
    return;
  }
  ...
  // === LOGO SEKOLAH ===
  const logoBase64 = await loadImageAsBase64(SCHOOL_LOGO);
  ...
  y += 15;
  pdf.setFontSize(11);
  pdf.text("Nama Sekolah : SMA IHBS Depok", 15, y);
  y += 7;
  pdf.text(`Tanggal      : ${lastResult.time}`, 15, y);

  // === FOTO HASIL ===
  y += 10;
  pdf.addImage(lastResult.image, "PNG", 15, y, 180, 90);
